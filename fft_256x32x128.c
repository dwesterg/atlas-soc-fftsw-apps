...
const char *g_test = "fft_256x32x128";
const char *g_description = "256x32x128 point FFT, FPGA, 16-bit input 24/32-bit output";
...
int main(int argc, char **argv)
{
	...
	fft_dev_fd = open("/dev/fft256", O_RDWR);
	if(fft_dev_fd < 0)
	...
	for(j = 0 ; j < FFT_IN_ROUNDS ; j++) {
		...
		/* perform FFT256 with FPGA hardware, 16 bit input 24/32 bit output */
